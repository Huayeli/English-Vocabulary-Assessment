...
- 核心等级：1K / 2K / 3K / 4K / 5K / 6K / 7K / 8K / 9K / 10K / 10K+（对应 BNC/COCA 词表，完整递进不跳档）
- 自适应测试：40 题、初始 3K、连对 4 题升级（按档重置计数）、连错 2 题降级、可返回修改答案
- 等级验证：指定等级 40 题，正确率 ≥80% 判定达标
- 四选一出题：1 正确 + 3 个与答案相关的混淆项，同场不重复
- 测试报告：等级、词汇量、各等级掌握率、CEFR、完成时间
- 防作弊：未完成会话自动放弃（不阻塞再次开测）、答题用时/选项分布/等级路径检测、结果可信度评分（<60 不计入平均词汇量、<40 提示疑似无效）、后台可手动标记无效
