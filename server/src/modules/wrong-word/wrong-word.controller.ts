...
export async function listHandler(req: AuthRequest, res: Response) {
  ...
  const data = await wrongWordService.listWrongWords(req.user!.userId, page, pageSize);
  res.json({ code: 0, message: "ok", data });
}

export async function removeHandler(req: AuthRequest, res: Response) {
  await wrongWordService.removeWrongWord(req.user!.userId, Number(req.params.id));
  res.json({ code: 0, message: "ok", data: null });
}

export async function reviewHandler(req: AuthRequest, res: Response) {
  await wrongWordService.reviewWrongWord(req.user!.userId, Number(req.params.id));
  res.json({ code: 0, message: "ok", data: null });
}
